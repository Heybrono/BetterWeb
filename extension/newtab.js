...
(function () {
  ...
  engineToggle.addEventListener('click', function (e) {
    ...
    if (ddOpen && searchSuggestions) searchSuggestions.classList.remove('open');
  });
  ...
  function doSearch() {
    ...
    if (!q) return;
    // Fire-and-forget save to not delay navigation
    addRecentQuery(q).catch(function () {});
    
    // Close UI immediately
    if (searchSuggestions) searchSuggestions.classList.remove('open');
    if (engineDrop) engineDrop.classList.remove('open');
  ...
  async function fetchSuggestions(query) {
    ...
    if (!q) return [];
    try {
      // Use Google suggestion API (firefox client returns clean JSON)
      var res = await fetch('https://suggestqueries.google.com/complete/search?client=firefox&q=' + encodeURIComponent(q));
      if (!res.ok) return [];
  ...
  function renderSuggestions(list) {
    ...
    list.slice(0, 8).forEach(function (txt, idx) {
      var li = document.createElement('li');
      li.className = 'suggestion-item';
      li.innerHTML = '<div class="suggestion-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="100%" height="100%"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div><div class="suggestion-text">' + esc(txt) + '</div>';
  ...
  function updateActiveSuggestion() {
    ...
    items.forEach(function (it, idx) {
      it.classList.toggle('active', idx === activeSuggestion);
    });
  }

  var onSearchInput = debounce(async function () {
    ...
    var list = await fetchSuggestions(q);
  ...
  searchInput.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') {
      if (activeSuggestion >= 0 && currentSuggestions[activeSuggestion]) {
        e.preventDefault();
        searchInput.value = currentSuggestions[activeSuggestion];
        doSearch();
      } else {
        doSearch();
      }
      ...
      if (currentSuggestions.length) {
        activeSuggestion++;
        if (activeSuggestion >= currentSuggestions.length) activeSuggestion = 0; // cycle
        updateActiveSuggestion();
      }
      ...
      if (currentSuggestions.length) {
        activeSuggestion--;
        if (activeSuggestion < 0) activeSuggestion = currentSuggestions.length - 1; // cycle
        updateActiveSuggestion();
      }
  ...
  $('#search-btn').addEventListener('click', function () { doSearch(); });
  // (Enter logic moved to main keydown handler above to support suggestion selection)
